...
import type { CompanyProfileInputDto, CompanyProfileDto } from '@milemoto/types';
...
function mapCompanyRow(row: CompanyRow | undefined): CompanyProfileDto | null {
  if (!row) return null;
  return {
    id: row.id,
    name: row.name,
...
/**
 * Get the company profile (singleton record with id=1)
 */
...
/**
 * Upsert (create or update) the company profile
 */
